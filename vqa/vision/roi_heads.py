...
class AttributePredictor(nn.Module):
    ...
    def loss(self, score, label):
        ...
        score = score.expand(n, self.max_attr_per_ins,
                             self.num_attributes).contiguous()
        score = score.view(-1, self.num_attributes)
        inv_weights = (
            (label >= 0).sum(dim=1).repeat(self.max_attr_per_ins, 1).transpose(
                0, 1).flatten()
        )
        ...
        label = label.view(-1)
        attr_loss = F.cross_entropy(score, label, reduction="none",
                                    ignore_index=-1)
...
class AttributeROIHeads(ROIHeads):
    ...
    def forward_attribute_loss(self, proposals, box_features):
        ...
        attribute_features = box_features[
            torch.cat(fg_selection_attributes, dim=0)]
        obj_labels = torch.cat([p.gt_classes for p in proposals])
        attribute_labels = torch.cat([p.gt_attributes for p in proposals],
                                     dim=0)
        attribute_scores = self.attribute_predictor(attribute_features,
                                                    obj_labels)
...
class AttributeRes5ROIHeads(AttributeROIHeads, Res5ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        if self.mask_on:
            self.mask_head = build_mask_head(
                cfg,
                ShapeSpec(channels=out_channels, width=pooler_resolution,
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        if self.training:
            ...
            if self.attribute_on:
                losses.update(
                    self.forward_attribute_loss(proposals, feature_pooled))
            return [], losses
        else:
...
class AttributeStandardROIHeads(AttributeROIHeads, StandardROIHeads):
    ...
    def _init_box_head(self, cfg, input_shape):
        ...
        self.box_head = build_box_head(
            cfg, ShapeSpec(channels=in_channels, height=pooler_resolution,
        ...
        if self.attribute_on:
            self.attribute_predictor = AttributePredictor(cfg,
                                                          self.box_head.output_shape.channels)
    ...
    def _forward_box(self, features, proposals):
        ...
        if self.training:
            ...
            if self.attribute_on:
                losses.update(
                    self.forward_attribute_loss(proposals, box_features))
                del box_features
